...
describe("BaseElement", () => {
  ...
  it("generates the correct shape styles", async () => {
    ...
    expect(test.styleHTML.values).toStrictEqual([
      "9999px",
      "unset",
      "unset",
      "unset",
    ...
    expect(test.styleHTML.values).toStrictEqual([
      "unset",
      `url(#${getSquircleClipPathID(0, DEFAULT_SQUIRCLE_CURVATURE)})`,
      "unset",
      "unset",
    ]);
    expect(test.shapeStyleInfo).toStrictEqual({
      clipPath: `url(#${getSquircleClipPathID(0, DEFAULT_SQUIRCLE_CURVATURE)})`,
    });
    ...
    expect(test.styleHTML.values).toStrictEqual([
      "unset",
      `url(#${getSquircleClipPathID(test.size, test.shapeSquircleCurvature)})`,
      "24px",
      "24px",
    ]);
    expect(test.shapeStyleInfo).toStrictEqual({
      clipPath: `url(#${getSquircleClipPathID(
        test.size,
        test.shapeSquircleCurvature
      )})`,
    });
  });
  ...
  it("generates the correct size styles", async () => {
    await render(test);
    expect(test.sizeStyleInfo).toStrictEqual({
      height: undefined,
      width: undefined,
    });
